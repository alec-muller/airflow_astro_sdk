...
# init dag
@dag(
    dag_id="demo-aws-users-json-snowflake",
...
def load_files_warehouse():
    ...
    users_json_files = aql.load_file(
        ...
        input_file=File(path="s3://alec-bucket/users/", filetype=FileType.JSON,
                        conn_id=AWS_CONN_ID),
        output_table=Table(name="users", conn_id=SNOWFLAKE_CONN_ID), # metadata=Metadata(schema="public", database="alec_db")),
        if_exists="replace",    
